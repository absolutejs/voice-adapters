...
const buildSetupMessage = (config: GeminiLiveAdapterOptions): GeminiClientMessage => {
	...
	open: (options) => {
		...
		ws.addEventListener('message', (event) => {
			try {
				...
				const outputText = content.outputTranscription?.text ?? extractText(message);
		...
		ws.addEventListener('close', (event) => {
			...
			clearReadyTimeout();
			if (!ready) {
				failReady(new Error('Gemini Live session closed before it became ready.'));
			}
			void emitClose(
				event.code,
