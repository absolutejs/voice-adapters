...
const normalizeWords = (
  ...
  const speaker = [...speakerCounts.entries()].sort(
...
type DeepgramOpenContext = Pick<
  DeepgramAdapterOpenOptions,
  "languageStrategy" | "lexicon" | "sessionId"
> & {
  ...
  const options = buildLiveOptions(
    {
      ...config,
      keyterms:
        selectedKeyterms.length > 0 ? selectedKeyterms : config.keyterms,
    },
...
const resolveOpenFailure = (error: unknown, url: string, timeoutMs: number) => {
