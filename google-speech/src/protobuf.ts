...
export const encodeVarint = (value: number): Uint8Array => {
  if (!Number.isFinite(value) || value < 0) {
    throw new Error(
      `encodeVarint requires a non-negative finite number, got ${String(value)}.`,
    );
  }
  const bytes: number[] = [];
  let remaining = value;
...
export const encodeTag = (fieldNumber: number, wireType: number): Uint8Array =>
...
export const encodeFloat = (
  ...
  return concat([encodeTag(fieldNumber, WIRE_I32), new Uint8Array(buffer)]);
};
