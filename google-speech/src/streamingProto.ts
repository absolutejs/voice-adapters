...
const encodeRecognitionFeatures = (features: RecognitionFeatures): Uint8Array =>
  concatProto([
...
const encodeRecognitionConfig = (config: RecognitionConfig): Uint8Array => {
  const parts: Uint8Array[] = [
    encodeSubMessage(
      8,
      encodeExplicitDecodingConfig(config.explicitDecodingConfig),
...
export const encodeStreamingRecognizeRequest = (
  ...
  if ("audio" in request && request.audio !== undefined) {
    parts.push(encodeSubMessage(5, request.audio)); // audio (bytes)
  }
  return concatProto(parts);
};
  ...
  while (cursor.offset < cursor.bytes.byteLength) {
    ...
    if (fieldNumber === 1 && wireType === WIRE_LEN) {
      result.alternatives.push(
        decodeAlternative(decodeLengthDelimited(cursor)),
      );
      continue;
    }
