...
export type GoogleSpeechStreamTransport = {
	...
	onEnd: (
		handler: (trailers: Record<string, string>) => void
	) => () => void;
...
export type GoogleSpeechStreamTransportFactory = (
	...
	| Promise<GoogleSpeechStreamTransport>;
...
const defaultTransportFactory = async ({
	...
	const endHandlers = new Set<
		(trailers: Record<string, string>) => void
	>();
	...
	return {
		...
		open: async (openOptions) => {
			...
			const transportFactory =
				config.transport ?? defaultTransportFactory;
						...
						transcript: {
							...
							endedAtMs:
								result.resultEndOffsetSeconds !== undefined
									? Math.round(
											result.resultEndOffsetSeconds * 1_000
									  )
									: undefined,
				...
				streamingConfig: {
					...
					streamingFeatures: {
						enableVoiceActivityEvents:
							config.enableVoiceActivityEvents,
