...
const buildStreamingResult = (
	text: string,
	options: { isFinal?: boolean; languageCode?: string; confidence?: number } = {}
) => {
	...
	if (options.languageCode)
		parts.push(stringField(6, options.languageCode));
	return concat(...parts);
};
		...
		expect(errors).toEqual([
			{
				code: '3',
				message:
					'Google Speech gRPC status 3: bad audio config'
			}
		]);
	});
