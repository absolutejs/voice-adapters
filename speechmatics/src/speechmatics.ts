...
export const speechmatics = (config: SpeechmaticsSTTOptions): STTAdapter => {
    ...
    open: async (openOptions) => {
      ...
      const openPromise = new Promise<void>((resolve, reject) => {
        ...
        socket.addEventListener(
          ...
          () => {
            ...
            try {
              socket.send(
                JSON.stringify(
                  buildStartRecognitionMessage(config, openOptions),
                ),
              );
            } catch (error) {
        ...
        socket.addEventListener("error", () => {
          clearTimeout(openTimeout);
          if (!recognitionStarted) {
            reject(new Error("Speechmatics websocket failed to open."));
          }
        });
