...
const resolveAcceptHeader = (outputFormat: PlayHTOutputFormat): string => {
  switch (outputFormat) {
    case "mp3":
      return "audio/mpeg";
    case "wav":
...
const resolveAudioFormat = (config: PlayHTTTSOptions): AudioFormat => {
...
const buildRequestPayload = (config: PlayHTTTSOptions, text: string) =>
  omitUndefined({
