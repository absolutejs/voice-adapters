...
const buildHttpUrl = (config: ElevenLabsTTSOptions) => {
  ...
  const url = new URL(`${STREAM_URL}/${config.voiceId}/stream`);
...
const buildWebSocketUrl = (config: ElevenLabsTTSOptions) => {
  const outputFormat = config.outputFormat ?? "pcm_16000";
  const url = new URL(`${WEBSOCKET_URL}/${config.voiceId}/stream-input`);
  url.searchParams.set("output_format", outputFormat);
...
export const elevenlabs = (config: ElevenLabsTTSOptions): TTSAdapter => ({
  kind: "tts",
  open: () => {
    ...
    if (config.transport === "websocket") {
      ...
      const sendSocketMessage = (payload: Record<string, unknown>) => {
        if (!socket || socket.readyState !== WebSocket.OPEN) {
          throw new Error("ElevenLabs WebSocket is not open for sending.");
        }
      ...
      const startKeepAlive = () => {
        ...
        keepAliveTimer = setInterval(() => {
          ...
          if (Date.now() - lastOutboundAt < keepAliveIntervalMs) {
            return;
          }
      ...
      ws.addEventListener("open", () => {
        try {
          sendSocketMessage(
            omitUndefined({
              generation_config: config.websocket?.chunkLengthSchedule
                ? {
                    chunk_length_schedule: config.websocket.chunkLengthSchedule,
                  }
                : undefined,
              text: " ",
              voice_settings: resolveVoiceSettings(config),
      ...
      ws.addEventListener("message", (event) => {
        void (async () => {
          try {
            ...
            if (message.error) {
              const failure = new Error(resolveErrorMessage(message.error));
      ...
      ws.addEventListener("error", () => {
        const failure = new Error("ElevenLabs WebSocket transport failed.");
        rejectReady(failure);
        rejectPendingGeneration(failure);
        void emit(listeners, "error", {
          error: failure,
      ...
      return {
        ...
        cancel: async (reason?: string) => {
          ...
          rejectPendingGeneration(new Error(reason ?? "cancelled"));
    ...
    return {
      ...
      send: async (text: string) => {
        ...
        try {
          ...
          const bytesPerSample = audioFormat.encoding === "pcm_s16le" ? 2 : 1;
