...
import type { GladiaEncoding, GladiaSTTOptions } from "./types";
...
const resolveEncoding = (format: { encoding?: string }): GladiaEncoding => {
...
const resolveBitDepth = (format: { encoding?: string }): number => {
  switch (format.encoding) {
    case "pcm_s16le":
      return 16;
    case "pcm_mulaw":
...
export const gladia = (config: GladiaSTTOptions): STTAdapter => {
  ...
  return {
    ...
    open: async (openOptions) => {
      ...
      const sessionBody =
        (await sessionResponse.json()) as GladiaSessionResponse;
      if (!sessionBody?.url) {
        throw new Error(
      ...
      socket.addEventListener("message", (event) => {
        ...
        if (type === "transcript" && data?.utterance) {
          ...
          const event: keyof STTSessionEventMap = isFinal ? "final" : "partial";
        ...
        if (type === "error") {
          const message =
            (parsed as unknown as { data?: { message?: string } }).data
              ?.message ?? "Gladia error";
