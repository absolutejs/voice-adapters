...
import type { RimeAudioFormat, RimeSampleRate, RimeTTSOptions } from "./types";
...
const resolveAcceptHeader = (audioFormat: RimeAudioFormat): string => {
  switch (audioFormat) {
    case "mulaw":
      return "audio/basic";
    case "pcm":
...
const buildRequestPayload = (config: RimeTTSOptions, text: string) =>
  omitUndefined({
