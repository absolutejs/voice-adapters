...
describe("lmnt TTS adapter", () => {
  test("posts to /v1/ai/speech/stream with X-API-Key and streams audio body", async () => {
    ...
    expect(calls[0]?.url).toBe("https://api.lmnt.com/v1/ai/speech/stream");
  ...
  test("rejects mp3 / wav format with a clear error", () => {
    expect(() => lmnt({ apiKey: "k", format: "mp3", voice: "lily" })).toThrow(
      /"raw" for PCM playback or "mulaw" for telephony/,
    );
  });
