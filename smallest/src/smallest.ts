...
import type { SmallestSampleRate, SmallestTTSOptions } from "./types";
...
const resolveStreamUrl = (config: SmallestTTSOptions): URL => {
  ...
  return new URL(`${resolveBaseUrl(config)}${DEFAULT_ENDPOINT_BUILDER(model)}`);
};
...
const buildRequestPayload = (config: SmallestTTSOptions, text: string) =>
  omitUndefined({
