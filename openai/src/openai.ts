...
const readTranscriptionLogprobs = (
  ...
  Array.isArray(value)
    ? value.filter(
        (entry): entry is OpenAITranscriptionLogprob =>
  ...
  const transcription =
    config.inputTranscriptionModel === null
      ...
      : omitUndefined({
          ...
          model: config.inputTranscriptionModel ?? DEFAULT_TRANSCRIPTION_MODEL,
...
export const openai = (
  ...
  open: (options) => {
    ...
    const runtimeConfig =
      phraseHintPrompt || lexiconPrompt
        ? {
            ...config,
    ...
    const primaryUpdate = buildPrimarySessionUpdate(
      runtimeConfig,
      runtimeOptions,
    );
    let ready = false;
      ...
      try {
        ...
        switch (payload.type) {
          ...
          case "conversation.item.input_audio_transcription.completed": {
            ...
            const completedLogprobs = readTranscriptionLogprobs(
              payload.logprobs,
            );
