...
export const openaiWhisper = (config: OpenAIWhisperSTTOptions): STTAdapter => {
  if (!config.apiKey) {
    throw new Error("@absolutejs/voice-openai-whisper requires an apiKey.");
  }
  ...
  return {
    ...
    open: (openOptions) => {
      ...
      const bitsPerSample = resolveBitsPerSample(openOptions.format.encoding);
      ...
      const flush = async () => {
        ...
        buffers.length = 0;
        bufferedBytes = 0;
        const wav = buildWavBuffer(pcm, sampleRateHz, channels, bitsPerSample);
        const file = new File(
...
export type OpenAIWhisperSession =
  ReturnType<ReturnType<typeof openaiWhisper>["open"]> extends Promise<infer S>
