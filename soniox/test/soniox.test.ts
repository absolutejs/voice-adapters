...
const baseConfig = {
  apiKey: "soniox-key",
  webSocket: {
    factory: (url: string) => new FakeWebSocket(url) as unknown as WebSocket,
  },
} as const;
...
describe("soniox STT adapter", () => {
  ...
  test("maps non-final tokens to partial events and final tokens to final events", async () => {
    ...
    socket.receiveText({
      tokens: [{ is_final: false, text: "hello" }],
    });
    socket.receiveText({
      tokens: [
  ...
  test("requires apiKey at construction", () => {
    expect(() => soniox({} as unknown as Parameters<typeof soniox>[0])).toThrow(
      /apiKey/,
    );
  });
});
