...
import type { AzureTTSOptions, AzureTTSOutputFormat } from "./types";
...
const DEFAULT_OUTPUT_FORMAT: AzureTTSOutputFormat = "raw-24khz-16bit-mono-pcm";
...
const parseSampleRate = (format: AzureTTSOutputFormat): number => {
...
const resolveRawSampleRate = (format: AzureTTSOutputFormat): number => {
...
const resolveAudioFormat = (format: AzureTTSOutputFormat): AudioFormat => {
  const lower = format.toLowerCase();
  if (!lower.startsWith("raw-")) {
    throw new Error(
...
const buildSsmlPayload = (config: AzureTTSOptions, text: string): string => {
