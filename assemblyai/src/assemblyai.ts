...
const normalizeWords = (words: AssemblyAITurnWord[] | undefined) => {
	...
	return {
		endedAtMs:
			typeof last?.end === 'number' ? Math.round(last.end) : undefined,
		startedAtMs:
	...
	open: (options) => {
		...
		const waitForOpen = new Promise<void>((resolve, reject) => {
			...
			ws.addEventListener('error', (rawEvent) => {
				...
				reject(event.error ?? new Error('AssemblyAI websocket failed to open'));
			}, { once: true });
		});
